...
<</SYS>>
### Context:\n{example['context']} 
[/INST] """.strip(), 'label': str({'question': example['question'], 'answer': ans, 'answer_start': start})}
...
def main(args_file=None):
    ...
    # Set project name
    os.environ["WANDB_PROJECT"] = "question-generation2"
    ...
    if model_args.model_type == 'llama':
        ...
        train_dataset = ds.map(generate_prompt)
        ds = load_dataset('squad_v2', split='validation')
        valid_dataset = ds.map(generate_prompt)
