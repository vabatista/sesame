...
class DataTrainingArguments:
    ...
    model_type: str = field(metadata={"help": "One of 't5', 'bart', 'llama'"})
...
class DataProcessor:
    ...
    def _add_special_tokens(self, example):
        ...
        example['source_text'] = example['source_text'].replace("{ha_token}", self.ha_token)  
...
def main():
    ...
    elif data_args.model_type == 'llama':
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b-chat-hf")
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.padding_side = "right"


    tokenizer.add_tokens(['<sep>', '<hl>', '<ha>'])
    ...
    train_dataset = nlp.load_dataset(data_args.dataset_path, 'highlight_qg_format', split=nlp.Split.TRAIN)
    valid_dataset = nlp.load_dataset(data_args.dataset_path, 'highlight_qg_format', split=nlp.Split.VALIDATION)
