    ...
    **kwargs,
    ...
    if isinstance(tokenizer, (str, tuple)):
        if isinstance(tokenizer, tuple):
            ...
            tokenizer = AutoTokenizer.from_pretrained(tokenizer[0], **tokenizer[1], device_map='auto')
        else:
            tokenizer = AutoTokenizer.from_pretrained(tokenizer, device_map='auto')
    ...
    if task == "question-generation":
        if ans_model is None:
            ...
            ans_tokenizer = AutoTokenizer.from_pretrained(ans_model, device_map='auto')
            ans_model = AutoModelForSeq2SeqLM.from_pretrained(ans_model, device_map='auto')
        else:
            # Try to infer tokenizer from model or config name (if provided as str)
            ...
            if isinstance(ans_tokenizer, (str, tuple)):
                if isinstance(ans_tokenizer, tuple):
                    # For tuple we have (tokenizer name, {kwargs})
                    ans_tokenizer = AutoTokenizer.from_pretrained(ans_tokenizer[0], **ans_tokenizer[1], device_map='auto')
                else:
                    ans_tokenizer = AutoTokenizer.from_pretrained(ans_tokenizer, device_map='auto')
