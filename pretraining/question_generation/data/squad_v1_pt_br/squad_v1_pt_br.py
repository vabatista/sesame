...
class SquadMultitask(nlp.GeneratorBasedBuilder):
    ...
    def _info(self):
        return nlp.DatasetInfo(
            ...
            homepage="https://github.com/arthurcbaia/squad-v1.1-pt-br/raw/main/",
            citation=_CITATION,
        )
    ...
    def _get_correct_alignement(self, context, answer):
        ...
        if start_idx == 0:
            try:
                if re.search(gold_text, context, re.IGNORECASE):
                    start_idx = re.search(gold_text, context, re.IGNORECASE).start()
            except:
                start_idx = 0
    ...
    def process_qa_text(self, context, question, answer):
        ans_gen_input = f"pergunta: {question}  contexto: {context}"
    ...
    def process_qg_text(self, context, question, answer):
        ...
        if self.config.qg_format == "prepend":
            que_gen_input = f"resposta: {answer_text}  contexto: {context}"
        elif self.config.qg_format == "highlight":
            start_pos, end_pos = self._get_correct_alignement(context, answer)
            que_gen_input = f"gerar pergunta: {context[:start_pos]} {{hl_token}} {answer_text} {{hl_token}} {context[end_pos:]}"
        else:
            start_pos, end_pos = self._get_correct_alignement(context, answer)
            que_gen_input = f"responder: {answer_text} contexto: {context[:start_pos]} {{hl_token}} {answer_text} {{hl_token}} {context[end_pos:]}"
